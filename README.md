# Sprinter Signing Deployment Guide
This guide is a step-by-step manual that will help you deploy Sprinter Signing nodes and join the Sprinter Stash credit system MPC consensus.
...
1. Prepare a Private key that will be used for MPC operations.
**Note: Never use this private key for something else and keep it secret**

2. For each network, you should have an RPC provider. Signing partners must align with the Sprinter team on the specific clients or RPC providers to use.
...
This document provides step-by-step instructions for deploying Signers with GitHub actions on the infrastructure on AWS,
as well as all necessary scripts to provision the infra and deploy the signers.
...
The only requirement to create the Signers infrastructure is a VPC. Of course, you can use the default VPC from any region in your AWS account, but it's a best practice to create a VPC to allocate your resources. If you don't know what a VPC is, [use this document to better understand it](https://docs.aws.amazon.com/vpc/latest/userguide/what-is-amazon-vpc.html).
...
| This dns directory is not tracked by git to avoid accidental exposure.

#### Configuration script

The configuration for the signers is in the folder `ecs`. For ECS we configure our applications with a Task Definition. [You can read more about it here](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/task_definitions.html).

**Change EFS configuration according to your provision results.**

[efsVolumeConfiguration](https://github.com/sprintertech/sygma-relayer-deployment/blob/main/ecs/task_definition_PARTNERS.j2#L227) - set this value to the EFS File system ID that was created (should have `signers-efs-Demo` name by default)
...
Set the next secret variables in GitHub Secret variables section. They will be used for deployment.
 ...
 `AWS_REGION` - a region where you want your signer to be deployed
 `AWS_ARN` - ARN of the user that will be deploying the Signer
...
You should see something like `"message":"Signer not part of MPC. Waiting for refresh event..."` that means that your signer is not part of MPC group! Hence move to the next section to proceed!

### Launching a signer to existing MPC set

After all the configuration parameters above are set and your Signer is running your Signer(s) need to be added to the Sprinter MPC network by updating the network topology.

Provide the Sprinter team with the next params so we update the topology map:

1. The network address. This could be a DNS name or IP address.
2. Peer ID - determined from libp2p private key being used for that signer (you can use signers [CLI command](https://github.com/sprintertech/sprinter-signing/blob/main/cli/peer/info.go) for this `peer info --private-key=<pk>`)

The final address of your signer in the topology map will look this `"/dns4/signer2/tcp/9001/p2p/QmeTuMtdpPB7zKDgmobEwSvxodrf5aFVSmBXX3SQJVjJaT"`

After all the information is provided Sprinter team will regenerate Topology Map and initiate key Resharing.

## Other

#### To change the number of signers to deploy
Amount of IDS in array set amount of signers to be deployed.

#### Signer shared configuration
Signer configuration is done with `--config-url` flag on Signer start and can be changed [here](https://github.com/sprintertech/sygma-relayer-deployment/blob/main/ecs/task_definition_PARTNERS.j2#L23)
This flag sets up shared configuration IPNS URL that is used by all Signers in the MPC network and provided by Sprinter.
...
```
2. Log Configuration - configure the Signer container with this lines of codes
...
We use OpenTelemetry Agent as a sidecar container for aggregating signers metrics, for now. 
...
- Sprinter will share the values of these variables through secure channel(s)


#### The Integration of the OpenTelemetry Agent
See the task Definition section for the integration [here](https://github.com/sprintertech/sygma-relayer-deployment/blob/main/ecs/task_definition_PARTNERS.j2#L199)
...
#### Sharing metrics with Sprinter
For sharing metrics you would need to use DataDog API key provided by Sprinter team. Set this key to DD_API_KEY env variable.  In task definition we are using ssm secret store for [this](https://github.com/sprintertech/sygma-relayer-deployment/blob/31a2a02d678d3f6940b09ac4876efe158495e950/ecs/task_definition_PARTNERS.j2#L165)

#### Private Repository Access
Configure [this](https://github.com/sprintertech/sygma-relayer-deployment/blob/main/ecs/task_definition_PARTNERS.j2#L201) as per your organisation.
You may chose to remove [this](https://github.com/sprintertech/sygma-relayer-deployment/blob/main/ecs/task_definition_PARTNERS.j2#L201) for accessing private repository.


The Sprinter Team Highly recommend to be security conscious while storing the shared credentials - store the credentials in private and secure environment with least previlige. Use Vault, AWS secrets manager for storing crednetials. 
